...
`,
    ...
    link(scope, element) {
        return scope.getClassObj = function(wd) {
            ...
            return ((() => {
                const result = []
                for (const [x, y] of Array.from(_.toPairs(output))) {
                         if (y) {
                        result.push(x)
                    }
                }
...
korpApp.directive("tabHash", (utils, $location, $timeout) => ({
    link(scope, elem, attr) {
        ...
        const watchHash = () => utils.setupHash(s,[{
            expr : "activeTab",
            val_out(val) {
                return val
            },
        ...
        $timeout(function() {
            s.fixedTabs = {}
            s.maxTab = -1
            for (const tab of Array.from(contentScope.tabset.tabs)) {
...
korpApp.directive("popper", $rootElement => ({
    ...
    link(scope, elem, attrs) {
        ...
        elem.on("click", function(event) {
            const other = $(".popper-menu:visible").not(popup)
            if (other.length) {
                other.hide()
            }
            if (popup.is(":visible")) {
...
korpApp.directive("extendedList", ($location, $rootScope) => ({
    ...
    link($scope, elem, attr) {
        ...
        s.removeToken = function(i) {
            ...
            for (const token of Array.from(s.data)) {
    ...
    link(s, elem, attr) {
        ...
        return w = s.$watchGroup(["dateModel", "timeModel"], function(...args) {
            ...
            if (date && time) {
                ...
                for (const t of Array.from(time_units)) {
        ...
        scope.$watchCollection('items', function() {
            if (scope.items) {
                ...
                if (scope.selected && (scope.selected.length > 0)) {
                    for (const select of Array.from(scope.selected)) {
                ...
                if (scope.insensitive) {
                    for (const insensitive of Array.from(scope.insensitive)) {
...
angular.module("template/datepicker/day.html", []).run($templateCache => $templateCache.put("template/datepicker/day.html", `\
...
angular.module("template/datepicker/month.html", []).run($templateCache => $templateCache.put("template/datepicker/month.html", `\
<table role="grid" aria-labelledby="{{uniqueId}}-title" aria-activedescendant="{{activeDateId}}">
<thead>
  <tr>
    <th><button type="button" class="btn btn-default btn-sm pull-left" ng-click="move(-1)" tabindex="-1"><i class="fa fa-chevron-left"></i></button></th>
...
angular.module("template/datepicker/year.html", []).run($templateCache => $templateCache.put("template/datepicker/year.html", `\
<table role="grid" aria-labelledby="{{uniqueId}}-title" aria-activedescendant="{{activeDateId}}">
<thead>
  <tr>
    <th><button type="button" class="btn btn-default btn-sm pull-left" ng-click="move(-1)" tabindex="-1"><i class="fa fa-chevron-left"></i></button></th>
...
angular.module("template/timepicker/timepicker.html", []).run($templateCache => $templateCache.put("template/timepicker/timepicker.html", `\
<table>
 <tbody>
