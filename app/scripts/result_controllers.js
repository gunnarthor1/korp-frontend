        ...
        const massageData = function(hitArray) {
            ...
            for (let i = 0; i < hitArray.length; i++) {
                ...
                for (let j = 0, end1 = hitContext.tokens.length, asc = end1 >= 0; asc ? j < end1 : j > end1; asc ? j++ : j--) {
                    ...
                    wd._open = []
                    wd._close = []
                    for (const { start, end } of Array.from(matches)) {
                        if (start <= j && j < end) {
...
korpApp.directive("statsResultCtrl", () => ({
    controller($scope, utils, $location, backend, searches, $rootScope) {
        ...
        s.getGeoAttributes = function(corpora) {
            let attrs = {}
            for (const corpus of Array.from(settings.corpusListing.subsetFactory(corpora).selected)) {
                for (const attr of Array.from(corpus.private_struct_attributes)) {
        ...
        return s.showMap = function() {
            ...
            const cqpExprs = {}
            for (const rowIx of Array.from(s.instance.getSelectedRows())) {
                if (rowIx === 0) {
                    continue
                }
...
korpApp.directive("wordpicCtrl", () => ({
    controller($scope, $rootScope, $location, utils, searches) {
        ...
        $rootScope.$on("word_picture_data_available", function(event, data) {
            ...
            let max = 0
            _.map(data, form => _.map(form, function(categories) {
                if (categories instanceof Array) {
                    return _.map(categories, cols => _.map(cols, function(col) {
        ...
        return s.promise.then(function(...args) {
            ...
            return s.rowClick = function(row, cmp_index) {
                ...
                var tokens = _.map(__range__(0, tokenLength - 1, true), function(tokenIdx) {
                           tokens = _.map(reduce, (reduceAttr, attrIdx) => _.uniq(_.map(splitTokens, res => res[attrIdx][tokenIdx])
                           ))
                           return tokens
                })
