...
util.loadCorporaFolderRecursive = function(first_level, folder) {
    ...
    if (first_level) {
        ...
        for (let val in settings.corpora) {
            ...
            for (const usedid in added_corpora_ids) {
...
util.loadCorpora = function() {
    ...
    window.corpusChooserInstance = $("#corpusbox").corpusChooser({
        template: outStr,
        infoPopup(corpusID) {
            let baseLangParagraphHTML, baseLangTokenHTML, lang
...
`
            ...
            let output = `\
<b>
    <img class="popup_icon" src="${samLogo}" />
    ${corpusObj.title}
</b>
${maybeInfo}
...
<br/><br/>`
        ...
        infoPopupFolder(indata) {
            ...
            if (missingParagraphData) { totalParagraphsString += "+" }
            let maybeInfo = ""
